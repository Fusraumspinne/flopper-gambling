...
import { PlayArrow, SkipNext, ArrowUpward, ArrowDownward } from "@mui/icons-material";
...
export default function HiloPage() {
  ...
  const multipliers = useMemo(() => {
    ...
    return {
      ...
      lower: lowerOrEqual > 0 ? Number((houseEdge / lowerOrEqual).toFixed(2)) : 0,
    };
  }, [probabilities]);
  ...
  const guess = (direction: "higher" | "lower") => {
    ...
    setTimeout(() => {
      ...
      if (won) {
        const stepMult = direction === "higher" ? multipliers.higher : multipliers.lower;
  ...
  const renderCard = (card: Card, isLarge = false) => {
    return (
      <div
        ...
        `}
      >
        <div className={`self-start font-bold ${isLarge ? "text-2xl" : "text-lg"} leading-none`}>
          {card.rank}
        </div>
        <div className={`${isLarge ? "text-6xl" : "text-3xl"}`}>{getSuitIcon(card.suit)}</div>
        <div className={`self-end font-bold ${isLarge ? "text-2xl" : "text-lg"} leading-none rotate-180`}>
          {card.rank}
        </div>
      </div>
    );
