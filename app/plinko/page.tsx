...
const getMultipliers = (rows: number, risk: RiskLevel): number[] => {
	...
	for (let i = 0; i < count; i++) {
		...
		const valRaw = minMulti + (maxMulti - minMulti) * Math.pow(t, exponent * (rows / 10));
		let val = valRaw;
...
export default function PlinkoPage() {
	...
	const draw = useCallback((time: number) => {
		...
		for (const ball of balls) {
			...
			ball.x = oneMinusT * oneMinusT * p0.x + 2 * oneMinusT * t * p1x + t * t * p2.x;
			ball.y = oneMinusT * oneMinusT * p0.y + 2 * oneMinusT * t * p1y + t * t * p2.y;
	...
	useEffect(() => {
		if (autoDrop) {
			...
			autoTimerRef.current = window.setInterval(() => {
				if (betAmount > 0 && betAmount <= balance) dropBall();
			}, 200);
		} else {
	...
	}, [autoDrop, betAmount, balance, rows]);
	...
	return (
		<div className="p-6 max-w-[1400px] mx-auto flex flex-col lg:flex-row gap-8">
			<div className="w-full lg:w-[350px] flex flex-col gap-6 bg-[#0f212e] p-6 rounded-xl h-fit">
				...
				<div className="space-y-2">
					<label className="text-xs font-bold text-[#b1bad3] uppercase tracking-wider">Rows: {rows}</label>
				...
				<div className="grid grid-cols-2 gap-2">
					...
					<button
						...
						className={`w-full py-3 rounded-md font-bold text-sm flex items-center justify-center gap-2 transition-colors ${
							autoDrop ? "bg-[#6b21a8] text-white" : "bg-[#2f4553] text-[#b1bad3] hover:bg-[#3e5666]"
						}`}
					>
						<Bolt sx={{ fontSize: 18 }} /> {autoDrop ? "Auto Drop On" : "Auto Drop"}
					</button>
				</div>
			...
			<div className="flex-1 bg-[#0f212e] p-6 rounded-xl min-h-[600px] flex items-center justify-center relative overflow-hidden">
				<div className="absolute top-4 left-1/2 -translate-x-1/2 text-center">
					...
					<div className="text-xs text-[#b1bad3] uppercase">{risk.charAt(0).toUpperCase() + risk.slice(1)}</div>
				</div>
				<canvas ref={canvasRef} width={900} height={650} className="w-full h-full object-contain" />
			</div>
		</div>
	);
}
